       ...
       if($level==2){
            ...
            <td>
                  <div style='font-weight:bold;'>
                    <i>
                    <a  class='t_detail_resume ".$bg_st."' data-id='$id_resume_medis' href='javascript:void()'>".strtoupper($status)."</a>
                    </i>
                  </div>
          </td>
          <td>
            <a href='#' onclick='' class='e_resume_medis' data-id='$id_resume_medis'>
              <i class='fa fa-pencil-square-o'></i>
            </a>
            <a href='javascript:hapus_resume_medis(\"{$id_resume_medis}\")' class='text-danger'>
 ...
 $(".t_detail_resume").click(function(){var id = $(this).attr("data-id");
    ...
    event.preventDefault();
      ...
      $.ajax({
        type  : "POST",
        url   : url,
        data  : "id_resume_medis="+id_resume_medis+"&status="+status,
        success : function(data){
          $("#m_detail").modal("hide");
            location.reload();
          //alert(data);
